...
async function runDaemon(targetId) {
  const pfPath = portFilePath(targetId, RUNTIME_DIR);
  ...
  // Localhost TCP server — NDJSON protocol with auth token
  ...
  const server = net.createServer((conn) => {
    ...
    conn.on('data', (chunk) => {
      ...
      const authLine = buf.slice(0, newlineIndex);
      buf = buf.slice(newlineIndex + 1);
      try {
        const message = JSON.parse(authLine);
        const expected = Buffer.from(authToken, 'utf8');
        const provided = Buffer.from(message?.token || '', 'utf8');
...
async function getOrStartTabDaemon(targetId) {
  ...
  // Wait for daemon info (includes time for user to click Allow)
  for (let i = 0; i < DAEMON_CONNECT_RETRIES; i++) {
    await sleep(DAEMON_CONNECT_DELAY);
    const daemonInfo = readPortFile(pfPath);
    if (!daemonInfo) continue;
    try { return await connectToDaemon(daemonInfo); } catch {}
  }
...
async function stopDaemons(targetPrefix) {
  const daemons = listDaemonPortFiles(RUNTIME_DIR);
  const targetIds = targetPrefix
    ? [resolvePrefix(targetPrefix, daemons.map((daemon) => daemon.targetId), 'daemon')]
    : daemons.map((daemon) => daemon.targetId);

  for (const targetId of targetIds) {
    const daemon = daemons.find((item) => item.targetId === targetId);
    if (!daemon) continue;
    try {
